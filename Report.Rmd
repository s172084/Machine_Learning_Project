...
Therefore it may be interesting to narrow in on the dataset and only look at diamonds that are between 1 and 5 carat, and exclude all diamonds below 1 carat.
...
selected the 99.99 percentile, which corresponds to the highest observations of
...
*y* and *z*, as already computed in the Table 3. In addition, also the *price*
...
The figure on the left shows that the first 3 principal components explain 
...
the *price*. It seems that these five characteristics alone explain two-thirds of the
variability of the diamonds. As for the second PC, it focuses more on the
cut characteristics (*depth* and *table*). The third PC mainly describes the cut characteristics
as well, but considering them as contibuting to the same direction.
:::
::::::

Data can be projected in the three-dimensional space defined by the PCs. 
By projecting them, some interesting features can be observed, as shown by the 3D graph below.
...
are mixed together. "Ideal" diamonds are concentrated in a very visible cluster, as for "fair" diamonds that
...
only a sample of 1000 data is plotted. No pattern and clusters can be seen from the figures, so it seems that
...
diamonds having low *carat* and low *price* are more numerous than the most expensive and heavy ones.
...
and the *carat*, meaning that the bigger and heavier diamonds are also more expensive. All these connections reflect to
...
somehow clustered. Unfortunately, the same is not valid for the other twp categorical attributes (*color* and *clarity*) that seem to be
...
Answer **B**: the attribute $x_1$ is ordinal because it can be said that time-interval number 5 is greater than
the time-interval number 4 since it is later in the day but it makes no sense to sum and subtract them
...
Answer **B**: according to the Sum Rule we know that $p(x_2|y)=\sum_{i=0}^1 p(x_2,x_7=i|y)$, so
...
Looking at the node $p(x_2=0|y=2)$, it can be seen that the probability is 0.84
