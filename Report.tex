...
\subsubsection{Introduction to the
...
Looking at the Table 2, standard deviation of \emph{price} is some
orders of magnitude larger than the others, so the dataset has to be
standardized by subtracting the mean and dividing by the standard
deviation of the whole set of observations of that variable.
...
The figure on the left shows that the first 3 principal components
...
two-thirds of the variability of the diamonds. As for the second PC, it
focuses more on the cut characteristics (\emph{depth} and \emph{table}).
The third PC mainly describes the cut characteristics as well, but
considering them as contibuting to the same direction.
...
by the 3D graph below. Data are colored by the attribute \emph{cut}

\begin{figure}
\centering
\includegraphics{Images/PC_Image.png}
\caption{alt-text-1}
\end{figure}
